...
def _screen_meta_id(idx: int) -> str:
    """Convert 0-based screen index → Meta positional ID: 0→SCREEN_A, 1→SCREEN_B, ..."""
    letters, n = "", idx + 1
    while n > 0:
        n, rem = divmod(n - 1, 26)
...
async def send_message(
    ...
    # ── MEDIA ─────────────────────────────────────────────────────────────────
        ...
        cap  = (body.caption or legacy.get("caption", "")).strip()
        fn   = (body.filename or legacy.get("filename", "")).strip()

        if mid:        obj["id"]   = mid
        elif link:     obj["link"] = link

        # caption: valid for image, video, document only — NOT audio or sticker
        # also only include when non-empty (WhatsApp rejects empty string)
        if cap and mtype in ("image", "video", "document"):
            obj["caption"] = cap

        # filename: ONLY valid for document
        if fn and mtype == "document":
            obj["filename"] = fn
        ...
        resp = await client._post(f"{client.base}/{client.phone_id}/messages", payload)
        # Store 'id' so frontend proxy can find the media_id the same way as inbound messages
        msg_content = {"id": mid or link, "caption": cap, "filename": fn, "mime_type": ""}

    # ── FLOW ──────────────────────────────────────────────────────────────────
    elif body.msg_type == "flow":
        meta_flow_id = body.flow_id.strip()
        if not meta_flow_id:
            raise HTTPException(400, "flow_id is required")
        if not meta_flow_id.isdigit():
            raise HTTPException(400, f"'{meta_flow_id}' is not a valid Meta flow ID (must be numeric). Select a flow that has been published on Meta.")

        # Map stored screen ID → positional Meta ID (SCREEN_A, SCREEN_B, ...)
        # Published flows use positional IDs generated by screens_to_flow_json.
        # If flow not in our DB (manually entered flow_id), default to SCREEN_A.
        flow_doc = await db.flows.find_one({"meta_flow_id": meta_flow_id, "tenant_id": tid})
        stored_screens = (flow_doc or {}).get("screens", [])
        target_stored  = body.flow_screen.strip()
        if target_stored and stored_screens:
            idx = next((i for i, s in enumerate(stored_screens) if s.get("id") == target_stored), 0)
        else:
            idx = 0
        # Always produce a valid screen ID — SCREEN_A is always the entry screen
        first_screen = _screen_meta_id(idx) if stored_screens else "SCREEN_A"
